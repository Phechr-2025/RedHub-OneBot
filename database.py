...
def refresh_selected_backend_if_needed() -> None:
    ...
    with _EXTERNAL_REFRESH_LOCK:
        if now < _SHEET_BACKOFF_UNTIL or now - _LAST_EXTERNAL_REFRESH < float(os.getenv("GOOGLE_SHEET_AUTO_REFRESH_SECONDS", "60")) or _SYNCING:
            return
...
def _google_credentials_configured() -> bool:
    """Return whether Google credentials exist without raising or calling API."""
    try:
        env_value = os.getenv("GOOGLE_SERVICE_ACCOUNT_JSON", "").strip()
        saved_value = str(get_setting("google_service_account_json", "") or "").strip()
        return bool(env_value or saved_value)
    except Exception:
        return False


def _run_scheduled_backend_sync() -> None:
    """Timer target that never leaks an exception into threading."""
    try:
        _sync_selected_backend_now()
    except Exception as exc:
        ...
        for table in tables:
            ...
            if worksheet is None:
                try:
                    ...
                    # Metadata can be stale after a prior successful addSheet.
