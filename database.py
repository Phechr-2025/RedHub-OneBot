...
def init_db():
    ...
    conn.execute("INSERT OR IGNORE INTO settings VALUES ('cancel_button_delay_seconds', '0')")
...
def get_recent_activity(limit: int = 100) -> List[Dict[str, Any]]:
    ...
    conn = _get_conn()
    rows = conn.execute("""SELECT id,user_id,username,code_name,network,days,gb,cost,link,created_at,'purchase' AS activity_type FROM buy_log
        UNION ALL SELECT id,user_id,username,code_name,network,0 AS days,gb,0 AS cost,link,created_at,'trial' AS activity_type FROM free_log
