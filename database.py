...
def init_db():
    ...
    # Starter packages keep the upgraded bot usable immediately. They can be
    # changed from either the web control panel or Telegram admin menu.
    if conn.execute("SELECT COUNT(*) AS n FROM products").fetchone()["n"] == 0:
        conn.executemany(
            "INSERT INTO products (name, description, days, gb_limit, price, network, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)",
            [
                ("เริ่มต้น 7 วัน", "เหมาะสำหรับทดลองใช้งาน", 7, 20, 20, "BOTH", 10),
                ("คุ้มค่า 30 วัน", "แพ็กเกจยอดนิยม ไม่จำกัดปริมาณ", 30, 0, 60, "BOTH", 20),
                ("ยาวนาน 90 วัน", "ใช้งานต่อเนื่อง ราคาประหยัด", 90, 0, 150, "BOTH", 30),
            ],
        )
    conn.commit()
...
def get_dashboard_overview() -> Dict[str, Any]:
    ...
    active_products = conn.execute("SELECT COUNT(*) AS n FROM products WHERE active=1").fetchone()["n"]
    ...
    return {"users": users, "orders": orders, "revenue": float(revenue),
            "active_products": active_products, "recent_orders": [dict(r) for r in recent]}
