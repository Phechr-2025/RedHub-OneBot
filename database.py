...
def create_package(network_id: int, name: str, description: str,
                   price_per_day: float, active: bool, sort_order: int) -> int:
    conn = _get_conn()
    cur = conn.execute(
        """INSERT INTO network_packages
           (network_id, name, description, price_per_day, active, sort_order)
           VALUES (?, ?, ?, ?, ?, ?)""",
        (network_id, name.strip(), description.strip(), price_per_day, int(active), sort_order),
    )
    conn.commit(); package_id = int(cur.lastrowid); conn.close()
    return package_id


def update_package(package_id: int, name: str, description: str,
                   price_per_day: float, active: bool, sort_order: int) -> bool:
    conn = _get_conn()
    cur = conn.execute(
        """UPDATE network_packages SET name=?, description=?, price_per_day=?, active=?,
           sort_order=?, updated_at=CURRENT_TIMESTAMP WHERE id=?""",
        (name.strip(), description.strip(), price_per_day, int(active), sort_order, package_id),
    )
    conn.commit(); changed = cur.rowcount > 0; conn.close()
    return changed
