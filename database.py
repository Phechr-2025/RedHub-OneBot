...
def _sync_selected_backend_now() -> None:
    ...
    try:
        ...
        if not sheet_id:
            logger.warning("Google Sheet sync skipped: google_sheet_id is empty")
            return
        backup_path = create_database_backup()
        import_database_file_to_google_sheet(backup_path, sheet_id, tab)
        logger.info("Local database synchronized to Google Sheet")
    except Exception as exc:
        # Never roll back a successful local SQLite write because Sheets is
        # temporarily unavailable; the next local commit retries the sync.
...
def sync_selected_backend_now() -> None:
    """Synchronize immediately; used by the web panel after important edits."""
