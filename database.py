...
def refresh_selected_backend_if_needed() -> None:
    """Refresh external Sheet data at a safe interval before database reads."""
    global _LAST_EXTERNAL_REFRESH, _AUTO_REFRESH_IN_PROGRESS, _SHEET_BACKOFF_UNTIL
    if _AUTO_REFRESH_IN_PROGRESS:
        return
    now = time.monotonic()
    with _EXTERNAL_REFRESH_LOCK:
        interval = float(os.getenv("GOOGLE_SHEET_AUTO_REFRESH_SECONDS", "60"))
        if now < _SHEET_BACKOFF_UNTIL or now - _LAST_EXTERNAL_REFRESH < interval or _SYNCING:
            return
        _LAST_EXTERNAL_REFRESH = now
        _AUTO_REFRESH_IN_PROGRESS = True
    try:
        if get_database_backend() != "google_sheet" or not _google_credentials_configured():
            return
        _sync_selected_backend_now()
    except Exception as exc:
        if "429" in str(exc) or "Quota exceeded" in str(exc):
            logger.warning("Google Sheet quota limit reached; automatic refresh paused for 5 minutes")
            _SHEET_BACKOFF_UNTIL = time.monotonic() + 300
        else:
            logger.warning("External database refresh failed: %s", exc)
    finally:
        _AUTO_REFRESH_IN_PROGRESS = False
...
def restore_database_from_file(source_path: str) -> None:
    ...
    # A backup contains application data, not the currently selected storage
    # destination. Keep the destination across replace/restore operations.
    preserved_backend = get_database_backend()
    preserved_sheet_id = str(get_setting(GOOGLE_SHEET_ID_KEY, "") or "").strip()
    preserved_sheet_tab = str(get_setting(GOOGLE_SHEET_TAB_KEY, "BioShop") or "BioShop").strip() or "BioShop"
    preserved_credentials = str(get_setting("google_service_account_json", "") or "")
    ...
    with suspend_backend_sync():
        set_setting(DATABASE_BACKEND_KEY, preserved_backend)
        set_setting(GOOGLE_SHEET_ID_KEY, preserved_sheet_id)
        set_setting(GOOGLE_SHEET_TAB_KEY, preserved_sheet_tab)
        set_setting("google_service_account_json", preserved_credentials)
...
def _google_credentials_configured() -> bool:
    try:
        return bool(os.getenv("GOOGLE_SERVICE_ACCOUNT_JSON", "").strip() or str(get_setting("google_service_account_json", "") or "").strip())
    except Exception:
        return False


def _run_scheduled_backend_sync() -> None:
    try:
        _sync_selected_backend_now()
    except Exception as exc:
        logger.warning("Scheduled database synchronization skipped: %s", exc)
...
def _schedule_selected_backend_sync() -> None:
    ...
    global _SYNC_TIMER
    try:
        if get_database_backend() != "google_sheet" or _SYNCING or not _google_credentials_configured():
            return
    except Exception:
        return
    with _SYNC_LOCK:
        ...
        _SYNC_TIMER = threading.Timer(delay, _run_scheduled_backend_sync)
...
def _sync_selected_backend_now() -> Dict[str, Any]:
    ...
    try:
        ...
        pull = bool(sheet_has_data and (not last_hash or local_hash == last_hash))
        if pull:
...
def get_database_backend_info() -> Dict[str, Any]:
    return {
        ...
        "google_service_account_json": os.getenv("GOOGLE_SERVICE_ACCOUNT_JSON", "").strip() or str(get_setting("google_service_account_json", "") or ""),
    }
...
def import_database_file_to_google_sheet(source_path: str, spreadsheet_id: str, tab_prefix: str = "BioShop") -> Dict[str, int]:
    ...
    try:
        ...
        worksheet_map = {str(item.title): item for item in spreadsheet.worksheets()}
        for table in tables:
...
def pull_database_from_google_sheet(spreadsheet_id: str, tab_prefix: str = "BioShop") -> Dict[str, Any]:
    ...
    try:
        ...
        raise ValueError(f"เปิด Google Sheet ไม่สำเร็จ: {exc}") from exc
    prefix = (tab_prefix or "BioShop").strip()
    # The Sheet's settings table is data, but backend selection is runtime
    # configuration. Preserve it so pulling a Sheet cannot silently switch the
    # application back to SQLite or erase the connection fields.
    preserved_backend = get_database_backend()
    preserved_sheet_id = str(get_setting(GOOGLE_SHEET_ID_KEY, "") or "").strip()
    preserved_sheet_tab = str(get_setting(GOOGLE_SHEET_TAB_KEY, "BioShop") or "BioShop").strip() or "BioShop"
    preserved_credentials = str(get_setting("google_service_account_json", "") or "")
    if spreadsheet_id:
        preserved_sheet_id = str(spreadsheet_id).strip()
    if tab_prefix:
        preserved_sheet_tab = prefix
    ...
    with _DB_FILE_LOCK, suspend_backend_sync():
        ...
        try:
            ...
            conn.close()
    init_db()
    # Restore runtime-only selection after importing the Sheet settings table.
    # This makes the choice survive page reloads, logout/login, and database
    # restore operations.
    with suspend_backend_sync():
        set_setting(DATABASE_BACKEND_KEY, preserved_backend)
        set_setting(GOOGLE_SHEET_ID_KEY, preserved_sheet_id)
