...
def _local_sync_hash() -> str:
    """Hash SQLite content in the same row/column shape exported to Sheets."""
    conn = _get_conn()
    try:
        payload: list[Any] = []
        ...
        for table in tables:
            ...
            payload.append([table, columns, normalized_rows])
...
def _google_sync_snapshot(spreadsheet_id: str, tab_prefix: str) -> tuple[str, bool]:
    """Return a content hash and whether the selected Sheet has database rows."""
    ...
    for worksheet in sorted(spreadsheet.worksheets(), key=lambda item: str(item.title)):
        ...
        rows = []
        for raw in values[1:]:
            row = [_sync_normalize(value) for value in list(raw) + [""] * max(0, len(headers) - len(raw))]
            row = row[:len(headers)]
...
def _sync_selected_backend_now() -> Dict[str, Any]:
    ...
    try:
        ...
        if local_hash == sheet_hash:
            ...
            return {"status": "ok", "direction": "already_equal", "hash": local_hash}

        # After the first common snapshot, detect which side changed. If both
        # changed, local SQLite wins because it is the live bot/web runtime.
        ...
        _set_sync_hash(common_hash)
        return {"status": "ok", "direction": "web_to_sheet", "hash": common_hash}
    except Exception as exc:
