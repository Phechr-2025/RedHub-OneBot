...
async def addclient_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sent = await update.message.reply_text(
        ...
        parse_mode="Markdown", reply_markup=_product_keyboard(user.id),
    )
    context.user_data["waiting_msg_id"] = sent.message_id
...
async def choose_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if product["network"] in {"AIS", "TRUE"}:
        ...
        await query.edit_message_text(f"✅ เลือก *{product['name']}* • {product['network']}", parse_mode="Markdown")
...
def _admin_panel_text() -> str:
    ...
    return (
        ...
        f"📦 สินค้าเปิดขาย: {len(products)} รายการ" + web_line
    )
