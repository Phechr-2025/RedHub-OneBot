...
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup, ForceReply, CopyTextButton
...
BUILD_VERSION = "v54.6-confirm-reply-copy"
...
def _copy_link_keyboard(link: str) -> InlineKeyboardMarkup:
    """ปุ่ม Copy Text ของ Telegram: แตะครั้งเดียวเพื่อคัดลอกลิงก์ VPN."""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📋 คัดลอกโค้ด/ลิงก์", copy_text=CopyTextButton(text=str(link)))]
    ])
...
async def _send_force_reply_question(
    ...
    sent = await context.bot.send_message(
        ...
        # ไม่ใช้ selective เพื่อให้ Reply ทำงานสม่ำเสมอทั้งมือถือ เดสก์ท็อป
        # และทุกชนิดแชทที่ Telegram รองรับ
...
async def mycodes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ส่งแยกเป็นรายการเพื่อให้ทุกลิงก์มีปุ่ม Copy Text ของตัวเอง
    entries = []
    for code in codes:
        ...
        entries.append((entry, str(code["link"])))

    header = f"📄 *โค้ดของคุณ* (แสดง {len(codes)} รายการ | {sort_label}):"

    in_group = update.effective_chat.type != "private"

    if in_group:
        try:
            await context.bot.send_message(chat_id=user.id, text=header, parse_mode="Markdown")
            for entry, link in entries:
                await context.bot.send_message(
                    chat_id=user.id, text=entry, parse_mode="Markdown",
                    reply_markup=_copy_link_keyboard(link),
                )
            await update.message.reply_text(
        ...
        for entry, link in entries:
            await update.message.reply_text(
                entry, parse_mode="Markdown", reply_markup=_copy_link_keyboard(link)
            )
    _schedule_start_menu_after_finish(update, context, "mycodes")
...
async def choose_package(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _send_force_reply_question(
        ...
        "📝 กรุณาตั้งชื่อไฟล์:",
        user_id=owner_id,
        reply_to_message_id=context.user_data.get("command_msg_id"),
    )
    return ENTER_NAME
...
async def enter_gb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if confirmed:
        gb = float(context.user_data.pop("pending_gb", 0)); await update.callback_query.answer()
        # เปลี่ยนหน้าสรุปให้เหลือสถานะของปุ่มที่ผู้ใช้เลือกทันที
        await update.callback_query.edit_message_text("✅ ยืนยันการสร้าง")
    elif fixed:
    ...
    if not confirmed:
        ...
        markup = InlineKeyboardMarkup([[InlineKeyboardButton("✅ ยืนยันสร้างไฟล์", callback_data=f"confirm_buy_{update.effective_user.id}")], [InlineKeyboardButton("❌ ยกเลิก", callback_data=f"cancel_addclient_{update.effective_user.id}")]])
    ...
    processing=await _reply_to_update(update, context, "⏳ กำลังสร้างไฟล์และส่งทาง DM...")
    ...
    try:
        await context.bot.send_message(
            chat_id=user.id, text=text, parse_mode=None,
            disable_web_page_preview=True, reply_markup=_copy_link_keyboard(link),
        )
        db.set_dm_started(user.id, True)
        if update.effective_chat.type!="private": await processing.edit_text("✅ สร้างสำเร็จและส่งไฟล์ไปยัง DM แล้ว")
        else: await processing.delete()
    except Exception:
        db.set_dm_started(user.id, False)
        await processing.edit_text("⚠️ สร้างสำเร็จ แต่ส่ง DM ไม่ได้ กรุณาเปิดแชทกับบอทแล้วใช้ /mycodes")
...
async def cancel_addclient_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    context.user_data.clear()
    await query.edit_message_text("❌ ยกเลิก")
    if waiting_id and query.message and waiting_id != query.message.message_id:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="❌ ยกเลิกรายการแล้ว",
            reply_to_message_id=waiting_id,
        )
    return ConversationHandler.END
...
async def free_enter_gb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not confirmed:
        ...
        markup = InlineKeyboardMarkup([[InlineKeyboardButton("✅ ยืนยันสร้างไฟล์", callback_data=f"confirm_free_{update.effective_user.id}")], [InlineKeyboardButton("❌ ยกเลิก", callback_data=f"cancel_freeclient_{update.effective_user.id}")]])
        if update.callback_query:
            await update.callback_query.edit_message_text(summary, reply_markup=markup)
        else:
            await _reply_to_update(update, context, summary, reply_markup=markup)
    ...
    processing_msg = await _reply_to_update(update, context, "⏳ กำลังสร้างโค้ดทดลอง กรุณารอสักครู่...")
    ...
    if in_group:
        ...
        await _reply_to_update(
            update, context, code_text, parse_mode="Markdown",
            reply_markup=_copy_link_keyboard(link),
        )
...
async def cancel_freeclient_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting_id and query.message and waiting_id != query.message.message_id:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="❌ ยกเลิกการสร้างโค้ดทดลองแล้ว",
            reply_to_message_id=waiting_id,
        )
    return ConversationHandler.END
