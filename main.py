...
BUILD_VERSION = "v35.0-network-packages"
...
async def enter_gb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    inbound_id=int(network["inbound_id"]); inbound_info=xui.get_inbound(inbound_id)
...
def _admin_panel_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton("🌐 ดูเครือข่าย", callback_data=f"admin|networks|{user_id}"), InlineKeyboardButton("🔄 รีเฟรช", callback_data=f"admin|refresh|{user_id}")],
    ]
...
async def admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action.startswith("toggle:"):
        ...
        networks = db.get_networks(active_only=True)
        text = "🌐 เครือข่ายที่เปิดใช้งาน\n\n" + ("\n".join(f"• {n['name']} — Inbound {n['inbound_id']}" for n in networks) if networks else "ยังไม่มีเครือข่าย")
        await query.answer()
