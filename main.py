...
# ── /addclient conversation (product-driven) ─────────────────────────────────
def _product_keyboard(user_id: int) -> InlineKeyboardMarkup:
    rows = []
    for product in db.get_products(active_only=True):
        gb_text = f"{product['gb_limit']:g}GB" if product['gb_limit'] else "ไม่จำกัด"
        label = f"{product['name']} • {product['days']}วัน • {gb_text} • {product['price']:g} เครดิต"
        rows.append([InlineKeyboardButton(label, callback_data=f"product_{product['id']}_{user_id}")])
...
async def addclient_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type != "private" and not db.has_dm_started(user.id):
        ...
        await update.message.reply_text(
            "⚠️ กรุณาเริ่มแชทส่วนตัวกับบอทก่อน เพื่อรับลิงก์อย่างปลอดภัย",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💬 เปิดแชทส่วนตัว", url=f"https://t.me/{bot_username}?start=welcome")]]),
        )
        return ConversationHandler.END
    products = db.get_products(active_only=True)
    if not products:
        await update.message.reply_text("❌ ยังไม่มีแพ็กเกจที่เปิดขาย กรุณาติดต่อแอดมิน")
        return ConversationHandler.END
    context.user_data.clear()
    context.user_data.update(session_user_id=user.id, command_msg_id=update.message.message_id)
    sent = await update.message.reply_text(
        "🛍 เลือกแพ็กเกจที่ต้องการ\nราคาและรายละเอียดนี้ซิงก์จากระบบจัดการสินค้า",
        parse_mode=None, reply_markup=_product_keyboard(user.id),
    )
    context.user_data["waiting_msg_id"] = sent.message_id
...
async def choose_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    try:
        _, product_id, owner_id = query.data.split("_", 2)
        product_id, owner_id = int(product_id), int(owner_id)
    except (ValueError, IndexError):
        await query.answer("ข้อมูลสินค้าไม่ถูกต้อง", show_alert=True)
        return SELECT_PRODUCT
    if query.from_user.id != owner_id:
        await _reject_foreign_interaction(update, is_callback=True)
        return SELECT_PRODUCT
    product = db.get_product(product_id, active_only=True)
    if not product:
        await query.answer("สินค้านี้ปิดขายหรือถูกลบแล้ว", show_alert=True)
        return SELECT_PRODUCT
    await query.answer()
    context.user_data["product"] = product
    if product["network"] in {"AIS", "TRUE"}:
        context.user_data["network"] = product["network"].lower()
        await query.edit_message_text(f"✅ เลือก {product['name']} • {product['network']}", parse_mode=None)
        await _send_force_reply_question(update, context, "📝 ตั้งชื่อโค้ด (ตัวอักษร/ตัวเลข/ขีด ไม่เกิน 50 ตัว):", user_id=owner_id, reply_to_message_id=context.user_data.get("command_msg_id"))
        return ENTER_NAME
    keyboard = [[
        InlineKeyboardButton("📶 AIS", callback_data=f"network_ais_{owner_id}"),
        InlineKeyboardButton("📶 TRUE", callback_data=f"network_true_{owner_id}"),
    ]]
    if _cancel_button_enabled():
        keyboard.append([InlineKeyboardButton("❌ ยกเลิก", callback_data=f"cancel_addclient_{owner_id}")])
    await query.edit_message_text(
        f"✅ แพ็กเกจ: {product['name']}\n🌐 เลือกเครือข่าย:",
        parse_mode=None, reply_markup=InlineKeyboardMarkup(keyboard),
    )
...
async def choose_network(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"✅ {product.get('name','แพ็กเกจ')} • {network.upper()}")
    await _send_force_reply_question(update, context, "📝 ตั้งชื่อโค้ด (ตัวอักษร/ตัวเลข/ขีด ไม่เกิน 50 ตัว):", user_id=owner_id, reply_to_message_id=context.user_data.get("command_msg_id"))
    return ENTER_NAME
...
async def enter_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db.code_name_exists(update.effective_user.id, name):
        await _send_force_reply_question(update, context, "❌ คุณมีโค้ดชื่อนี้แล้ว กรุณาใช้ชื่ออื่น:", user_id=update.effective_user.id, reply_to_message_id=update.message.message_id)
        return ENTER_NAME
    ...
    keyboard = [[InlineKeyboardButton("✅ ยืนยันการซื้อ", callback_data=f"buyconfirm_{product['id']}_{uid}")],
                [InlineKeyboardButton("❌ ยกเลิก", callback_data=f"cancel_addclient_{uid}")]]
    await update.message.reply_text(
        f"🧾 ตรวจสอบรายการ\n\nแพ็กเกจ: {product['name']}\nเครือข่าย: {context.user_data['network'].upper()}\nอายุ: {product['days']} วัน\nปริมาณ: {gb_text}\nราคา: {product['price']:g} เครดิต\nชื่อโค้ด: {name}",
        parse_mode=None, reply_markup=InlineKeyboardMarkup(keyboard),
    )
...
async def confirm_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.answer("ข้อมูลไม่ถูกต้อง", show_alert=True)
        return CONFIRM_PURCHASE
    if query.from_user.id != owner_id:
        await _reject_foreign_interaction(update, is_callback=True)
        return CONFIRM_PURCHASE
    # Re-read immediately before charging so web edits/disable take effect.
    product = db.get_product(product_id, active_only=True)
    if not product:
        await query.answer("สินค้านี้ปิดขายแล้ว ไม่มีการหักเครดิต", show_alert=True)
        context.user_data.clear()
        return ConversationHandler.END
    await query.answer()
    await query.edit_message_text("⏳ กำลังตรวจสอบและสร้างโค้ด...")
    user = query.from_user
    network = context.user_data.get("network")
    name = context.user_data.get("name")
    if network not in {"ais", "true"} or not name:
        await query.edit_message_text("❌ เซสชันไม่สมบูรณ์ กรุณาเริ่ม /addclient ใหม่")
        context.user_data.clear()
        return ConversationHandler.END
    if not xui.is_available():
        await query.edit_message_text("❌ ระบบ VPN Panel ไม่พร้อมใช้งาน ยังไม่มีการหักเครดิต")
        context.user_data.clear()
        return ConversationHandler.END
    cost, days, gb = float(product["price"]), int(product["days"]), float(product["gb_limit"])
    if not db.try_deduct_credit(user.id, cost):
        await query.edit_message_text(f"❌ เครดิตไม่เพียงพอ\nมี {db.get_credit(user.id):.2f} / ต้องใช้ {cost:.2f} เครดิต")
        context.user_data.clear()
        return ConversationHandler.END
    ...
    if not result:
        _refund_credit_after_failure(user.id, cost, "3x-ui add_client failed")
        await query.edit_message_text("❌ สร้าง Client ไม่สำเร็จ ระบบคืนเครดิตแล้ว")
        context.user_data.clear()
        return ConversationHandler.END
    link = xui.generate_link(inbound_id, result["uuid"], full_name, flow=result.get("flow", ""))
    if not link:
        xui.delete_client(full_name)
        _refund_credit_after_failure(user.id, cost, "3x-ui link generation failed")
        await query.edit_message_text("❌ สร้างลิงก์ไม่สำเร็จ ระบบคืนเครดิตแล้ว")
        context.user_data.clear()
        return ConversationHandler.END
    ...
    db.add_buy_log(user.id, user.username, full_name, network.upper(), days, gb, cost, link, _thai_now_str())
    db.add_audit_log(f"telegram:{user.id}", "order.create", full_name, "", f"product={product_id};cost={cost}")
    gb_text = f"{gb:g} GB" if gb else "ไม่จำกัด"
    text = (f"✅ สร้างโค้ดสำเร็จ\n\n📦 {product['name']}\n📌 {full_name}\n🌐 {network.upper()}\n📅 {expire_date} ({days} วัน)\n💾 {gb_text}\n💰 หัก {cost:.2f} • คงเหลือ {db.get_credit(user.id):.2f}\n\n🔗 {link}")
    if query.message.chat.type != "private":
        try:
            await context.bot.send_message(user.id, text, parse_mode=None)
            await query.edit_message_text("✅ สร้างสำเร็จ ส่งรายละเอียดไปยังแชทส่วนตัวแล้ว")
        except Exception:
            await query.edit_message_text("⚠️ สร้างสำเร็จแล้ว แต่ส่ง DM ไม่ได้ ใช้ /mycodes เพื่อดูโค้ด")
    else:
...
def _admin_panel_text() -> str:
    ...
    return (
        ...
        f"📦 สินค้าเปิดขาย: {len(products)} รายการ\n"
...
def _admin_panel_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton("📦 ดูสินค้า", callback_data=f"admin|products|{user_id}"), InlineKeyboardButton("🔄 รีเฟรช", callback_data=f"admin|refresh|{user_id}")],
    ]
...
async def admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action.startswith("toggle:"):
        ...
        products = db.get_products(active_only=True)
        text = "📦 สินค้าที่เปิดขาย\n\n" + ("\n".join(f"• {p['name']} — {p['days']} วัน / {p['gb_limit']:g} GB / {p['price']:g} เครดิต / {p['network']}" for p in products) if products else "ยังไม่มีสินค้า")
        await query.answer()
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            SELECT_PRODUCT: [
                CallbackQueryHandler(choose_product, pattern=r"^product_\d+_\d+$"),
            ...
            CONFIRM_PURCHASE: [
                CallbackQueryHandler(confirm_purchase, pattern=r"^buyconfirm_\d+_\d+$"),
                CallbackQueryHandler(cancel_addclient_callback, pattern=r"^cancel_addclient_\d+$"),
            ],
        },
        fallbacks=[
