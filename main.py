...
BUILD_VERSION = "v34.0-network-flow"
...
# ── /addclient conversation: network → name → days → GB → DM ────────────────
...
async def addclient_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not _addclient_enabled():
        await update.message.reply_text("❌ ระบบขาย VPN ปิดใช้งานชั่วคราว")
        return ConversationHandler.END
    ...
    if not networks:
        await update.message.reply_text("❌ ยังไม่มีเครือข่ายที่เปิดใช้งาน กรุณาติดต่อแอดมิน")
        return ConversationHandler.END
    context.user_data.clear()
    context.user_data.update(session_user_id=user.id, command_msg_id=update.message.message_id)
    sent = await update.message.reply_text(
        "🌐 กรุณาเลือกเครือข่าย:", parse_mode=None,
        reply_markup=_network_keyboard(user.id),
    )
    context.user_data["waiting_msg_id"] = sent.message_id
...
async def choose_network(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    try:
        _, network_id, owner_id = query.data.split("_", 2)
        network_id, owner_id = int(network_id), int(owner_id)
    except (ValueError, IndexError):
        await query.answer("ข้อมูลเครือข่ายไม่ถูกต้อง", show_alert=True)
        return CHOOSE_NETWORK
    ...
    if not network:
        await query.answer("เครือข่ายนี้ถูกปิดหรือลบแล้ว", show_alert=True)
        return CHOOSE_NETWORK
    await query.answer()
    context.user_data["network_record"] = network
    await query.edit_message_text(f"✅ เครือข่าย: {network['name']}", parse_mode=None)
    await _send_force_reply_question(
        update, context, "📝 กรุณาตั้งชื่อไฟล์ (ตัวอักษร/ตัวเลข/ขีด ไม่เกิน 50 ตัว):",
        user_id=owner_id, reply_to_message_id=context.user_data.get("command_msg_id"),
    )
    return ENTER_NAME
...
async def enter_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not re.match(r"^[\w\-\.]{1,50}$", name):
        await _send_force_reply_question(update, context, "❌ ชื่อไม่ถูกต้อง กรุณาลองใหม่:", user_id=update.effective_user.id, reply_to_message_id=update.message.message_id)
        return ENTER_NAME
    if db.code_name_exists(update.effective_user.id, name):
        await _send_force_reply_question(update, context, "❌ คุณมีชื่อนี้แล้ว กรุณาใช้ชื่ออื่น:", user_id=update.effective_user.id, reply_to_message_id=update.message.message_id)
        return ENTER_NAME
    context.user_data["name"] = name
    await _send_force_reply_question(
        update, context, "📅 กรุณาระบุจำนวนวัน (1–365 วัน):",
        user_id=update.effective_user.id, reply_to_message_id=update.message.message_id,
    )
    return ENTER_DAYS
...
async def enter_days(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await _send_force_reply_question(update, context, "❌ กรุณาระบุจำนวนวันเป็นเลข 1–365:", user_id=update.effective_user.id, reply_to_message_id=update.message.message_id)
        return ENTER_DAYS
    price_per_day = float(db.get_setting("price_per_day", 2.0))
    total_cost = round(days * price_per_day, 2)
    context.user_data.update(days=days, cost=total_cost)
    await _send_force_reply_question(
        update, context,
        f"📅 {days} วัน • ราคา {total_cost:.2f} เครดิต\n💾 กรุณาระบุ GB (ใส่ 0 = ไม่จำกัด):",
        user_id=update.effective_user.id, reply_to_message_id=update.message.message_id,
    )
    return ENTER_GB
...
async def enter_gb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await _send_force_reply_question(update, context, "❌ กรุณาระบุ GB เป็นเลข 0 ขึ้นไป:", user_id=update.effective_user.id, reply_to_message_id=update.message.message_id)
        return ENTER_GB
    ...
    if not network or not name or not days or cost is None:
        await update.message.reply_text("❌ เซสชันหมดอายุ กรุณาเริ่ม /addclient ใหม่")
        context.user_data.clear(); return ConversationHandler.END
    # อ่านเครือข่ายซ้ำก่อนหักเครดิต ให้ค่าจากเว็บมีผลทันที
    network = db.get_network(int(network["id"]), active_only=True)
    if not network:
        await update.message.reply_text("❌ เครือข่ายนี้ถูกปิดหรือลบแล้ว ยังไม่มีการหักเครดิต")
        context.user_data.clear(); return ConversationHandler.END
    if not xui.is_available():
        await update.message.reply_text("❌ ระบบ VPN Panel ไม่พร้อม ยังไม่มีการหักเครดิต")
        context.user_data.clear(); return ConversationHandler.END
    if not db.try_deduct_credit(user.id, float(cost)):
        await update.message.reply_text(f"❌ เครดิตไม่เพียงพอ\nมี {db.get_credit(user.id):.2f} / ต้องใช้ {float(cost):.2f} เครดิต")
        context.user_data.clear(); return ConversationHandler.END

    processing = await update.message.reply_text("⏳ กำลังสร้างไฟล์และส่งทาง DM...")
    inbound_id = int(network["inbound_id"])
    ...
    if not result:
        _refund_credit_after_failure(user.id, float(cost), "3x-ui add_client failed")
        await processing.edit_text("❌ สร้าง Client ไม่สำเร็จ ระบบคืนเครดิตแล้ว")
        context.user_data.clear(); return ConversationHandler.END
    link = xui.generate_link(inbound_id, result["uuid"], full_name, flow=result.get("flow", ""))
    if not link:
        xui.delete_client(full_name)
        _refund_credit_after_failure(user.id, float(cost), "3x-ui link generation failed")
        await processing.edit_text("❌ สร้างลิงก์ไม่สำเร็จ ระบบคืนเครดิตแล้ว")
        context.user_data.clear(); return ConversationHandler.END

    expire_date = (_thai_now() + datetime.timedelta(days=int(days))).strftime("%Y-%m-%d")
    network_label = str(network["name"])
    db.save_code(user.id, full_name, result["uuid"], inbound_id, network_label, expire_date, gb, link)
    db.add_buy_log(user.id, user.username, full_name, network_label, int(days), gb, float(cost), link, _thai_now_str())
    db.add_audit_log(f"telegram:{user.id}", "order.create", full_name, "", f"network={network['code']};cost={cost}")
    gb_text = f"{gb:g} GB" if gb else "ไม่จำกัด"
    text = (
        f"✅ สร้างไฟล์สำเร็จ\n\n📌 {full_name}\n🌐 {network_label}\n"
        f"📅 {expire_date} ({days} วัน)\n💾 {gb_text}\n"
        f"💰 หัก {float(cost):.2f} • คงเหลือ {db.get_credit(user.id):.2f}\n\n🔗 {link}"
    )
    try:
        await context.bot.send_message(chat_id=user.id, text=text, parse_mode=None, disable_web_page_preview=True)
        if update.effective_chat.type != "private":
            await processing.edit_text("✅ สร้างสำเร็จและส่งไฟล์ไปยัง DM แล้ว")
        else:
...
async def cancel_addclient_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer(); context.user_data.clear()
    await query.edit_message_text("❌ ยกเลิกรายการแล้ว")
    return ConversationHandler.END


async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data.clear(); await update.message.reply_text("❌ ยกเลิกรายการแล้ว")
    return ConversationHandler.END
