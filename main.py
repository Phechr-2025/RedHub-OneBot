...
async def startadmin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        ...
        return
    await update.message.reply_text(_admin_panel_text(), parse_mode="Markdown", reply_markup=_admin_panel_keyboard(update.effective_user.id), disable_web_page_preview=True)
...
async def admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action.startswith("toggle:"):
        ...
        await query.answer()
        await query.edit_message_text(text, parse_mode="Markdown", reply_markup=_admin_panel_keyboard(owner), disable_web_page_preview=True)
        return
    else:
        await query.answer("อัปเดตข้อมูลแล้ว")
    await query.edit_message_text(_admin_panel_text(), parse_mode="Markdown", reply_markup=_admin_panel_keyboard(owner), disable_web_page_preview=True)
