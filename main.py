...
async def choose_package(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await query.edit_message_text(f"✅ {network['name']} • {package['name']} • {package['price_per_day']:g} เครดิต/วัน")
    await _send_force_reply_question(
        update, context, "📝 กรุณาตั้งชื่อไฟล์:", user_id=owner_id,
...
async def enter_days(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fixed_gb > 0:
        ...
        sent = await update.message.reply_text(f"📅 {days} วัน • ราคา {cost:.2f} เครดิต\n💾 แพ็กเกจนี้กำหนดโควตา {fixed_gb:g} GB\nระบบจะใช้ค่านี้โดยไม่ถามซ้ำ", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ สร้างไฟล์ต่อ", callback_data=f"fixedgb_buy_{update.effective_user.id}")]]))
        context.user_data["waiting_msg_id"] = sent.message_id
    else:
        max_gb = float(package.get("gb_max") or 0)
        if max_gb > 0:
            gb_prompt = f"💾 กรุณาระบุ GB ที่ต้องการใช้ (สูงสุด {max_gb:g} GB):"
        else:
            gb_prompt = "💾 กรุณาระบุ GB ที่ต้องการใช้:\nหากไม่จำกัดพิมพ์ 0"
        await _send_force_reply_question(update, context, f"📅 {days} วัน • ราคา {cost:.2f} เครดิต\n{gb_prompt}", user_id=update.effective_user.id, reply_to_message_id=update.message.message_id)
    return ENTER_GB
...
async def enter_gb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fixed:
        ...
        try:
            ...
            limit_text = f"ตั้งแต่ 0 ถึง {max_gb:g} GB" if max_gb > 0 else "เป็นเลข 0 ขึ้นไป"
            await _send_force_reply_question(update, context, f"❌ กรุณาระบุค่า GB {limit_text}", user_id=update.effective_user.id, reply_to_message_id=update.message.message_id); return ENTER_GB
...
async def free_enter_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fixed_gb > 0:
        context.user_data["preset_gb"] = fixed_gb
        sent = await update.message.reply_text(f"💾 แพ็กเกจทดลองกำหนดโควตา {fixed_gb:g} GB\nระบบจะใช้ค่านี้โดยไม่ถามซ้ำ", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ สร้างไฟล์ต่อ", callback_data=f"fixedgb_free_{update.effective_user.id}")]]))
        context.user_data["waiting_msg_id"] = sent.message_id
        return FREE_ENTER_GB
    max_gb = float(free_package.get("gb_max") or 0)
    if max_gb > 0:
        gb_prompt = f"💾 กรุณาระบุ GB ที่ต้องการใช้สำหรับทดลอง (สูงสุด {max_gb:g} GB):"
    else:
        gb_prompt = "💾 กรุณาระบุ GB ที่ต้องการใช้สำหรับทดลอง:\nหากไม่จำกัดพิมพ์ 0"
    await _send_force_reply_question(
        update,
        context,
        gb_prompt,
    ...
    if fixed:
        ...
        try:
            ...
            await _send_force_reply_question(
                update,
                context,
                (f"❌ กรุณาระบุค่า GB ตั้งแต่ 0 ถึง {max_gb:g} GB" if max_gb > 0 else "❌ กรุณาระบุค่า GB เป็นตัวเลข 0 ขึ้นไป หากไม่จำกัดให้พิมพ์ 0"),
                user_id=update.effective_user.id,
                reply_to_message_id=update.message.message_id,
                flow_name="freeclient",
            )
