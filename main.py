...
BUILD_VERSION = "v36.0-package-inbound-order"
...
async def admin_panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action.startswith("toggle:"):
        ...
        networks = db.get_networks(active_only=True)
        text = "🌐 เครือข่ายและแพ็กเกจที่เปิดใช้งาน\n\n" + ("\n".join(
            f"{n['sort_order']}. {n['name']} (ID {n['id']}) — {n['active_package_count']} แพ็กเกจ" for n in networks
        ) if networks else "ยังไม่มีเครือข่าย") + "\n\nจัดการ Inbound ID และรายละเอียดแพ็กเกจได้จากเว็บไซต์"
        await query.answer()
