...
BUILD_VERSION = "v38.0-retention-backup-auto-order"
...
async def addmycredit_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type == "private":
        success_text = (
            ...
            f"💰 ค��ณมีเครดิตรวมทั้งหมด {_format_credit(balance)} เครดิต"
        )
    else:
        success_text = (
            f"✅ คุณได้เติมเครดิต {_format_credit(credit_amount)} เคร��ิต\n"
            f"💸 ยอดซองอั่งเปา: {_format_credit(amount_baht)} บาท"
        )
...
async def addclient_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type != "private" and not db.has_dm_started(user.id):
        bot_username = (await context.bot.get_me()).username
        await update.message.reply_text(
            "⚠️ กรุณาเริ่มแชทส่วนตัวกับบอทก่อน เพื่อรับไฟล์ทาง DM",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(
                "💬 เปิดแชทส่วนตัว", url=f"{{https://t.me/{bot_username}}}?start=welcome"
            )]]),
        )
        return ConversationHandler.END
...
async def addcode_unit_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "fixed":
        ...
        detail = (
            ...
            "การแจก: สุ่��เครดิตจากยอดคงเหลือของโค้ดจนกว่าจะเต็ม"
        )
        created_title = "✅ สร้างโค้ดเครดิตสำเร็จ"
    else:
        detail = (
            f"จำนวนผู้ใช้: {_format_limit_count(max_uses)}\n"
            "ผลลัพธ์เมื่อกรอก: รีเซ็ตจำ���วนการใช้สิทธิ์ทดลองใช้ฟรีของผู้ใช้เป็น 0"
        )
...
async def listaddclient(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            f"📋 จำนวนรายการที่แสดงใน /logbuy ปั��จุบัน: *{current}* รายการ\n\n"
