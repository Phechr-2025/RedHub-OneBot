...
BUILD_VERSION = "v40.0-live-dm-text-repair"
...
async def _send_cancel_button_before_question(
    ...
    # ถ้ามีคำถามถัดไปหรือจบ session แล้ว ข้อความยกเลิกของคำถามเก่า
    # ต้องไม่ถูกส่งย้อนมาปะปนกับข้อความปัจจุบัน
...
async def _send_force_reply_question(
    ...
    # ส่งคำถามก่อนเสมอ เพื่อให้ผู้ใช้เห็นสิ่งที่ต้องกรอกทันที
    # ปุ่มยกเลิกเป็นข้อความแยกและทำงานแบบ background: ค่าหน่วงมีผลเฉพาะ
    # เวลาที่ปุ่มยกเลิกถูกส่ง ไม่บล็อกคำถามหรือขั้นตอนของบริการ
    ...
    if _cancel_button_enabled():
        ...
        context.user_data["_cancel_button_task"] = context.application.create_task(
            _send_cancel_button_before_question(
                context, chat_id, user_id, flow_name, generation
            ),
            update=update,
            name=f"cancel_button_{flow_name}_{user_id}",
        )
    return sent
...
async def _reply_to_update(
    ...
    **kwargs,
    ...
    """ตอบข้อความได้ทั้ง Message update และ Inline callback update"""
...
async def choose_network(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not packages:
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("⬅️ เลือกเครือข่ายใหม่", callback_data=f"backnet_{owner_id}")],
                *([[InlineKeyboardButton("❌ ยกเลิก", callback_data=f"cancel_addclient_{owner_id}")]] if _cancel_button_enabled() else []),
            ]),
        )
...
async def enter_days(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fixed_gb > 0:
        ...
        fixed_keyboard = [[InlineKeyboardButton("✅ สร้างไฟล์ต่อ", callback_data=f"fixedgb_buy_{update.effective_user.id}")]]
        if _cancel_button_enabled():
            fixed_keyboard.append([InlineKeyboardButton("❌ ยกเลิก", callback_data=f"cancel_addclient_{update.effective_user.id}")])
        sent = await update.message.reply_text(f"📅 {days} วัน • ราคา {cost:.2f} เครดิต\n💾 แพ็กเกจนี้กำหนดโควตา {fixed_gb:g} GB\nระบบจะใช้ค่านี้โดยไม่ถามซ้ำ", reply_markup=InlineKeyboardMarkup(fixed_keyboard))
        context.user_data["waiting_msg_id"] = sent.message_id
    else:
        max_gb = float(package.get("gb_max") or 0)
        note = f" สูงสุด {max_gb:g} GB" if max_gb > 0 else ""
        await _send_force_reply_question(update, context, f"📅 {days} วัน • ราคา {cost:.2f} เครดิต\n💾 กรุณาระบุ GB{note} (0 = ไม่จำกัด):", user_id=update.effective_user.id, reply_to_message_id=update.message.message_id)
    return ENTER_GB
...
async def free_enter_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fixed_gb > 0:
        context.user_data["preset_gb"] = fixed_gb
        fixed_keyboard = [[InlineKeyboardButton("✅ สร้างไฟล์ต่อ", callback_data=f"fixedgb_free_{update.effective_user.id}")]]
        if _cancel_button_enabled():
            fixed_keyboard.append([InlineKeyboardButton("❌ ยกเลิก", callback_data=f"cancel_freeclient_{update.effective_user.id}")])
        sent = await update.message.reply_text(f"💾 แพ็กเกจทดลองกำหนดโควตา {fixed_gb:g} GB\nระบบจะใช้ค่านี้โดยไม่ถามซ้ำ", reply_markup=InlineKeyboardMarkup(fixed_keyboard))
        context.user_data["waiting_msg_id"] = sent.message_id
    ...
    await _send_force_reply_question(
        update,
        context,
        f"💾 *กรุณาระบุ GB ที่ต้องการจำกัดสำหรับทดลอง:*{max_note}\n_(หากไม่จำกัดพิมพ์ 0)_",
        user_id=update.effective_user.id,
        parse_mode="Markdown",
        reply_to_message_id=update.message.message_id,
        flow_name="freeclient",
