...
class PackageBody(BaseModel):
    # fixed = แพ็กเกจกำหนด GB เอง, user = ให้ผู้ใช้กำหนด GB
    gb_mode: str = Field(default="user", pattern=r"^(fixed|user)$")
...
def _package_values(body: PackageBody) -> dict[str, Any]:
    values = body.model_dump()
    mode = values.pop("gb_mode", "user")
    if mode == "fixed":
        if values["gb_limit"] <= 0:
            raise HTTPException(400, "กรุณาระบุจำนวน GB ที่กำหนดให้มากกว่า 0")
        # ค่าเพดานไม่มีความหมายเมื่อแพ็กเกจกำหนด GB ตายตัว
        values["gb_max"] = 0
    else:
        # โหมดให้ผู้ใช้กำหนดต้องเริ่มจาก 0; gb_max=0 หมายถึงไม่จำกัด
        values["gb_limit"] = 0
    return values
...
async function packagePage(networkId){let d=await api(`/api/networks/${networkId}/packages`);activeNetwork=d.network;packages=d.items;$('#title').textContent=`แพ็กเกจ • ${activeNetwork.name}`;$('#page').innerHTML=`<div class="panel"><div class="panel-head"><div><button class="btn" onclick="networkPage();$('#title').textContent='จัดการเครือข่าย'">← เครือข่าย</button> <b style="margin-left:10px">${esc(activeNetwork.name)}</b></div><button class="btn primary" onclick="packageModal()">+ เพิ่มแพ็กเกจ</button></div>${packageTable(packages)}</div>`}function packageTable(a){if(!a.length)return '<div class="empty"><b>ยังไม่มีรายการแพ็กเกจ</b><br><small>เพิ่มแพ็กเกจแรกเพื่อให้ผู้ใช้เลือกในบอท</small></div>';return `<div class="table-wrap"><table><thead><tr><th>แพ็กเกจ</th><th>Inbound ID (3x-ui)</th><th>ราคา/วัน</th><th>ลำดับ</th><th>สถานะ</th><th></th></tr></thead><tbody>${a.map(x=>`<tr><td><b>${esc(x.name)}</b><br><small class="package-description">${esc(x.description)||'—'}</small><br><small>${x.gb_limit>0?'กำหนด '+money(x.gb_limit)+' GB':'ให้ผู้ใช้กำหนด'+(x.gb_max>0?' (สูงสุด '+money(x.gb_max)+' GB)':' (ไม่จำกัด)')}</small></td><td>${x.inbound_id}</td><td>${money(x.price_per_day)} เครดิต</td><td>${x.sort_order}</td><td><span class="tag ${x.active?'on':''}">${x.active?'เปิด':'ปิด'}</span></td><td><button class="btn" onclick="packageModal(${x.id})">แก้ไข</button> <button class="btn danger" onclick="removePackage(${x.id})">ลบ</button></td></tr>`).join('')}</tbody></table></div>`}
function packageModal(id){let x=packages.find(p=>p.id===id)||{name:'',description:'',inbound_id:'',price_per_day:'',gb_limit:0,gb_max:0,active:true,sort_order:0};let mode=Number(x.gb_limit||0)>0?'fixed':'user';showModal(`<h2>${id?'แก้ไข':'เพิ่ม'}แพ็กเกจ • ${esc(activeNetwork.name)}</h2><div class="form-grid"><div class="field full"><label>ชื่อแพ็กเกจ</label><input id="pkname" value="${esc(x.name)}" placeholder="เช่น โปรกันรั่ว SSH"></div><div class="field full"><label>รายละเอียดแพ็กเกจ</label><textarea id="pkdesc" rows="18" placeholder="อธิบายเงื่อนไข ความเร็ว อายุการใช้งาน หรือข้อจำกัดของแพ็กเกจได้หลายบรรทัด">${esc(x.description)}</textarea><div class="gb-help">รายละเอียดจะแสดงให้ลูกค้าเห็นหลังเลือกแพ็กเกจ และรองรับการขึ้นบรรทัดใหม่</div></div><div class="field full"><label>รูปแบบจำนวน GB</label><div class="choice-group"><label class="choice"><input type="radio" name="gbMode" value="fixed" ${mode==='fixed'?'checked':''} onchange="toggleGbFields()"><strong>กำหนดจำนวน GB</strong><small>ระบบใช้จำนวนนี้ทันที ไม่ถามผู้ใช้ซ้ำ</small></label><label class="choice"><input type="radio" name="gbMode" value="user" ${mode==='user'?'checked':''} onchange="toggleGbFields()"><strong>ให้ผู้ใช้กำหนด</strong><small>กำหนดเพดานสูงสุดได้ หรือใส่ 0 เพื่อไม่จำกัด</small></label></div></div><div class="field" id="fixedGbField"><label>จำนวน GB</label><input id="pkgb" type="number" min="0.01" step="0.01" value="${x.gb_limit||0}"><small>จำนวนโควตาที่แพ็กเกจนี้กำหนดตายตัว</small></div><div class="field" id="maxGbField"><label>จำนวน GB สูงสุดที่ผู้ใช้ตั้งได้</label><input id="pkgbmax" type="number" min="0" step="0.01" value="${x.gb_max||0}"><small>0 = ไม่จำกัด</small></div><div class="field"><label>Inbound ID ใน 3x-ui</label><input id="pkinbound" type="number" min="1" value="${x.inbound_id}" placeholder="เช่น 1"></div><div class="field"><label>ราคาต่อวัน (เครดิต)</label><input id="pkprice" type="number" min="0.01" step="0.01" value="${x.price_per_day}"></div><div class="field"><label>ลำดับ (0 = อัตโนมัติ)</label><input id="pkorder" type="number" min="0" value="${x.sort_order}"></div><div class="field"><label>สถานะ</label><select id="pkactive"><option value="1" ${x.active?'selected':''}>เปิด</option><option value="0" ${!x.active?'selected':''}>ปิด</option></select></div></div><div class="actions"><button class="btn" onclick="closeModal()">ยกเลิก</button><button id="packageSave" class="btn primary" onclick="savePackage(${id||0})">บันทึก</button></div>`);toggleGbFields()}function toggleGbFields(){let mode=document.querySelector('input[name="gbMode"]:checked')?.value||'user',fixed=document.querySelector('#fixedGbField'),max=document.querySelector('#maxGbField'),gb=document.querySelector('#pkgb'),gbmax=document.querySelector('#pkgbmax');if(!fixed||!max)return;let isFixed=mode==='fixed';fixed.style.display=isFixed?'block':'none';max.style.display=isFixed?'none':'block';gb.required=isFixed;gbmax.required=false;}async function savePackage(id){let button=$('#packageSave');try{let name=$('#pkname').value.trim(),description=$('#pkdesc').value,inbound=Number($('#pkinbound').value),price=Number($('#pkprice').value),mode=document.querySelector('input[name="gbMode"]:checked')?.value||'user',gb_limit=mode==='fixed'?Number($('#pkgb').value||0):0,gb_max=mode==='user'?Number($('#pkgbmax').value||0):0,order=Number($('#pkorder').value||0);if(!name)throw Error('กรุณากรอกชื่อแพ็กเกจ');if(!Number.isInteger(inbound)||inbound<1)throw Error('Inbound ID ต้องเป็นเลขตั้งแต่ 1');if(!(price>0))throw Error('ราคาต่อวันต้องมากกว่า 0');if(mode==='fixed'&&!(gb_limit>0))throw Error('กรุณาระบุจำนวน GB ที่กำหนดให้มากกว่า 0');if(mode==='user'&&(gb_max<0||gb_max>1000000))throw Error('จำนวน GB สูงสุดไม่ถูกต้อง');if(!Number.isInteger(order)||order<0)throw Error('ลำดับต้องเป็นเลข 0 ขึ้นไป');button.disabled=true;await api(id?`/api/packages/${id}`:`/api/networks/${activeNetwork.id}/packages`,{method:id?'PUT':'POST',body:JSON.stringify({gb_mode:mode,name,description,inbound_id:inbound,price_per_day:price,gb_limit,gb_max,active:$('#pkactive').value==='1',sort_order:order})});closeModal();toast('บันทึกแพ็กเกจแล้ว');await packagePage(activeNetwork.id)}catch(e){toast(e.message||'บันทึกแพ็กเกจไม่สำเร็จ')}finally{if(button)button.disabled=false}}async function removePackage(id){try{if(!confirm('ลบแพ็กเกจนี้? ระบบจะจัดลำดับใหม่อัตโนมัติ'))return;await api(`/api/packages/${id}`,{method:'DELETE'});toast('ลบและจัดลำดับใหม่แล้ว');await packagePage(activeNetwork.id)}catch(e){toast(e.message||'ลบแพ็กเกจไม่สำเร็จ')}}
