...
const $=s=>document.querySelector(s), esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));let networks=[];
...
async function networkPage(){let d=await api('/api/networks');networks=d.items;$('#page').innerHTML=`<div class="panel"><div class="panel-head"><h2>เครือข่าย VPN</h2><button class="btn primary" onclick="networkModal()">+ เพิ่มเครือข่าย</button></div>${networkTable(networks)}</div>`}function networkTable(a){if(!a.length)return '<div class="empty">ยังไม่มีเครือข่าย กด “เพิ่มเครือข่าย” เพื่อเริ่มต้น</div>';return `<div class="table-wrap"><table><thead><tr><th>ชื่อ</th><th>รหัส</th><th>Inbound ID</th><th>ลำดับ</th><th>สถานะ</th><th></th></tr></thead><tbody>${a.map(x=>`<tr><td><b>${esc(x.name)}</b></td><td><span class="tag">${esc(x.code)}</span></td><td>${x.inbound_id}</td><td>${x.sort_order}</td><td><span class="tag ${x.active?'on':''}">${x.active?'เปิด':'ปิด'}</span></td><td><button class="btn" onclick="networkModal(${x.id})">แก้ไข</button> <button class="btn danger" onclick="removeNetwork(${x.id})">ลบ</button></td></tr>`).join('')}</tbody></table></div>`}
function networkModal(id){let x=networks.find(n=>n.id===id)||{code:'',name:'',inbound_id:1,active:true,sort_order:10};showModal(`<h2>${id?'แก้ไข':'เพิ่ม'}เครือข่าย</h2><div class="form-grid"><div class="field"><label>รหัส เช่น AIS</label><input id="nc" maxlength="24" value="${esc(x.code)}"></div><div class="field"><label>ชื่อที่แสดง</label><input id="nn" value="${esc(x.name)}"></div><div class="field"><label>Inbound ID (เริ่ม 1)</label><input id="ni" type="number" min="1" value="${x.inbound_id}"></div><div class="field"><label>ลำดับ (เริ่ม 1)</label><input id="ns" type="number" min="1" value="${x.sort_order}"></div><div class="field"><label>สถานะ</label><select id="na"><option value="1" ${x.active?'selected':''}>เปิด</option><option value="0" ${!x.active?'selected':''}>ปิด</option></select></div></div><div class="actions"><button class="btn" onclick="closeModal()">ยกเลิก</button><button class="btn primary" onclick="saveNetwork(${id||0})">บันทึก</button></div>`)}async function saveNetwork(id){let b={code:nc.value.trim().toUpperCase(),name:nn.value.trim(),inbound_id:+ni.value,active:na.value==='1',sort_order:+ns.value};await api('/api/networks'+(id?'/'+id:''),{method:id?'PUT':'POST',body:JSON.stringify(b)});closeModal();toast('บันทึกเครือข่ายแล้ว — บอทใช้ค่าใหม่ทันที');networkPage()}async function removeNetwork(id){if(!confirm('ลบเครือข่ายนี้?'))return;await api('/api/networks/'+id,{method:'DELETE'});toast('ลบเครือข่ายแล้ว');networkPage()}
