...
function toggleGbFields(){let mode=$('#pkgbmode')?.value||'user',fixed=$('#pkgbFixed'),max=$('#pkgbMax');if(fixed)fixed.classList.toggle('hidden',mode!=='fixed');if(max)max.classList.toggle('hidden',mode!=='user')}
function packageModal(id){let x=packages.find(p=>p.id===id)||{name:'',description:'',inbound_id:'',price_per_day:'',gb_limit:0,gb_max:0,active:true,sort_order:0},gbMode=Number(x.gb_limit||0)>0?'fixed':'user';showModal(`<h2>${id?'แก้ไข':'เพิ่ม'}แพ็กเกจ • ${esc(activeNetwork.name)}</h2><div class="form-grid"><div class="field full"><label>ชื่อแพ็กเกจ</label><input id="pkname" value="${esc(x.name)}" placeholder="เช่น โปรกันรั่ว SSH"></div><div class="field full"><label>รายละเอียด</label><textarea id="pkdesc" rows="10" placeholder="รายละเอียดแพ็กเกจ">${esc(x.description)}</textarea><small>สามารถกรอกหลายบรรทัดได้</small></div><div class="field"><label>Inbound ID ใน 3x-ui</label><input id="pkinbound" type="number" min="1" value="${x.inbound_id}" placeholder="เช่น 1"></div><div class="field"><label>ราคาต่อวัน (เครดิต)</label><input id="pkprice" type="number" min="0.01" step="0.01" value="${x.price_per_day}"></div><div class="field full"><label>การกำหนดจำนวน GB</label><select id="pkgbmode" onchange="toggleGbFields()"><option value="fixed" ${gbMode==='fixed'?'selected':''}>กำหนดจำนวน GB</option><option value="user" ${gbMode==='user'?'selected':''}>ให้ผู้ใช้กำหนด</option></select></div><div class="field" id="pkgbFixed"><label>จำนวน GB</label><input id="pkgb" type="number" min="0.01" step="0.01" value="${x.gb_limit||0}"><small>ระบบจะกำหนดค่านี้ให้ผู้ใช้โดยอัตโนมัติ</small></div><div class="field" id="pkgbMax"><label>จำนวน GB สูงสุดที่ผู้ใช้ตั้งได้</label><input id="pkgbmax" type="number" min="0" step="0.01" value="${x.gb_max||0}"><small>ใส่ 0 หากไม่จำกัด</small></div><div class="field"><label>ลำดับ (0 = อัตโนมัติ)</label><input id="pkorder" type="number" min="0" value="${x.sort_order}"></div><div class="field"><label>สถานะ</label><select id="pkactive"><option value="1" ${x.active?'selected':''}>เปิด</option><option value="0" ${!x.active?'selected':''}>ปิด</option></select></div></div><div class="actions"><button class="btn" onclick="closeModal()">ยกเลิก</button><button id="packageSave" class="btn primary" onclick="savePackage(${id||0})">บันทึก</button></div>`);toggleGbFields()}
async function savePackage(id){let button=$('#packageSave');try{let name=$('#pkname').value.trim(),description=$('#pkdesc').value,inbound=Number($('#pkinbound').value),price=Number($('#pkprice').value),gbMode=$('#pkgbmode').value,gb_limit=gbMode==='fixed'?Number($('#pkgb').value||0):0,gb_max=gbMode==='user'?Number($('#pkgbmax').value||0):0,order=Number($('#pkorder').value||0);if(!name)throw Error('กรุณากรอกชื่อแพ็กเกจ');if(!Number.isInteger(inbound)||inbound<1)throw Error('Inbound ID ต้องเป็นเลขตั้งแต่ 1');if(!(price>0))throw Error('ราคาต่อวันต้องมากกว่า 0');if(gbMode==='fixed'&&!(gb_limit>0))throw Error('กรุณากำหนดจำนวน GB มากกว่า 0');if(gbMode==='user'&&gb_max<0)throw Error('จำนวน GB สูงสุดต้องไม่ติดลบ');if(!Number.isInteger(order)||order<0)throw Error('ลำดับต้องเป็นเลข 0 ขึ้นไป');button.disabled=true;await api(id?`/api/packages/${id}`:`/api/networks/${activeNetwork.id}/packages`,{method:id?'PUT':'POST',body:JSON.stringify({name,description,inbound_id:inbound,price_per_day:price,gb_limit,gb_max,active:$('#pkactive').value==='1',sort_order:order})});closeModal();toast('บันทึกแพ็กเกจแล้ว');await packagePage(activeNetwork.id)}catch(e){toast(e.message||'บันทึกแพ็กเกจไม่สำเร็จ')}finally{if(button)button.disabled=false}}
function removePackage(id){try{if(!confirm('ลบแพ็กเกจนี้? ระบบจะจัดลำดับใหม่อัตโนมัติ'))return;await api(`/api/packages/${id}`,{method:'DELETE'});toast('ลบและจัดลำดับใหม่แล้ว');await packagePage(activeNetwork.id)}catch(e){toast(e.message||'ลบแพ็กเกจไม่สำเร็จ')}}
