...
SETTING_SCHEMA: dict[str, dict[str, Any]] = {
    ...
    "price_per_day": {"label": "ราคาต่อวัน (โหมดเดิม)", "type": "number", "default": "2", "min": 0.01, "group": "ราคา"},
...
class ProductBody(BaseModel):
    ...
    gb_limit: float = Field(ge=0, le=1_000_000)
    price: float = Field(ge=0, le=1_000_000)
    network: str = Field(default="BOTH")
    active: bool = True
...
def products(request: Request) -> dict[str, Any]:
    _require(request)
    return {"items": db.get_products(active_only=False)}
...
def create_product(body: ProductBody, request: Request) -> dict[str, Any]:
    ...
    if network not in {"AIS", "TRUE", "BOTH"}:
        raise HTTPException(400, "เครือข่ายไม่ถูกต้อง")
    product_id = db.create_product(**body.model_dump(exclude={"network"}), network=network)
    db.add_audit_log("web", "product.create", str(product_id), "", body.model_dump_json())
    return {"ok": True, "id": product_id}


@app.put("/api/products/{product_id}")
def update_product(product_id: int, body: ProductBody, request: Request) -> dict[str, Any]:
    _require(request)
    network = body.network.upper()
    if network not in {"AIS", "TRUE", "BOTH"}:
        raise HTTPException(400, "เครือข่ายไม่ถูกต้อง")
    if not db.update_product(product_id, **body.model_dump(exclude={"network"}), network=network):
        raise HTTPException(404, "ไม่พบสินค้า")
    db.add_audit_log("web", "product.update", str(product_id), "", body.model_dump_json())
    return {"ok": True}
...
def delete_product(product_id: int, request: Request) -> dict[str, Any]:
    ...
    if not db.delete_product(product_id):
        raise HTTPException(404, "ไม่พบสินค้า")
    db.add_audit_log("web", "product.delete", str(product_id), "active", "deleted")
    return {"ok": True}
...
async function productPage(){let d=await api('/api/products');products=d.items;$('#page').innerHTML=`<div class="panel"><div class="panel-head"><h2>แพ็กเกจ VPN</h2><button class="btn primary" onclick="productModal()">+ เพิ่มสินค้า</button></div>${productTable(products)}</div>`}function productTable(a){if(!a.length)return '<div class="empty">ยังไม่มีสินค้า กด “เพิ่มสินค้า” เพื่อเริ่มต้น</div>';return `<div class="table-wrap"><table><thead><tr><th>สินค้า</th><th>เครือข่าย</th><th>อายุ</th><th>ปริมาณ</th><th>ราคา</th><th>สถานะ</th><th></th></tr></thead><tbody>${a.map(x=>`<tr><td><b>${esc(x.name)}</b><br><small>${esc(x.description)}</small></td><td>${x.network}</td><td>${x.days} วัน</td><td>${x.gb_limit?x.gb_limit+' GB':'ไม่จำกัด'}</td><td>${money(x.price)}</td><td><span class="tag ${x.active?'on':''}">${x.active?'เปิดขาย':'ปิด'}</span></td><td><button class="btn" onclick="productModal(${x.id})">แก้ไข</button> <button class="btn danger" onclick="removeProduct(${x.id})">ลบ</button></td></tr>`).join('')}</tbody></table></div>`}
function productModal(id){let x=products.find(p=>p.id===id)||{name:'',description:'',days:30,gb_limit:0,price:50,network:'BOTH',active:true,sort_order:0};showModal(`<h2>${id?'แก้ไข':'เพิ่ม'}สินค้า</h2><div class="form-grid"><div class="field full"><label>ชื่อแพ็กเกจ</label><input id="pn" value="${esc(x.name)}"></div><div class="field full"><label>คำอธิบาย</label><textarea id="pd">${esc(x.description)}</textarea></div><div class="field"><label>จำนวนวัน</label><input id="pdays" type="number" min="1" value="${x.days}"></div><div class="field"><label>จำกัด GB (0 = ไม่จำกัด)</label><input id="pgb" type="number" min="0" value="${x.gb_limit}"></div><div class="field"><label>ราคาเครดิต</label><input id="pp" type="number" min="0" step=".01" value="${x.price}"></div><div class="field"><label>เครือข่าย</label><select id="pnet"><option ${x.network==='BOTH'?'selected':''}>BOTH</option><option ${x.network==='AIS'?'selected':''}>AIS</option><option ${x.network==='TRUE'?'selected':''}>TRUE</option></select></div><div class="field"><label>ลำดับ</label><input id="ps" type="number" value="${x.sort_order}"></div><div class="field"><label>สถานะ</label><select id="pa"><option value="1" ${x.active?'selected':''}>เปิดขาย</option><option value="0" ${!x.active?'selected':''}>ปิด</option></select></div></div><div class="actions"><button class="btn" onclick="closeModal()">ยกเลิก</button><button class="btn primary" onclick="saveProduct(${id||0})">บันทึก</button></div>`)}async function saveProduct(id){let b={name:pn.value,description:pd.value,days:+pdays.value,gb_limit:+pgb.value,price:+pp.value,network:pnet.value,active:pa.value==='1',sort_order:+ps.value};await api('/api/products'+(id?'/'+id:''),{method:id?'PUT':'POST',body:JSON.stringify(b)});closeModal();toast('บันทึกสินค้าแล้ว');productPage()}async function removeProduct(id){if(!confirm('ลบสินค้านี้? ประวัติเดิมจะยังอยู่'))return;await api('/api/products/'+id,{method:'DELETE'});toast('ลบสินค้าแล้ว');productPage()}
