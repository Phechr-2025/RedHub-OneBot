...
def change_database_backend(body: DatabaseBackendBody, request: Request) -> dict[str, Any]:
    ...
    if not body.confirmed:
        raise HTTPException(400, "กรุณายืนยันการเปลี่ยนฐานข้อมูล")
    ...
    if backend == "google_sheet":
        ...
        # Validate access before resetting local data, so a bad sheet cannot
    ...
    try:
        ...
        db.set_database_backend(backend, sheet_id, tab)
    ...
    return {"ok": True, "backend": backend, "google_sheet_id": sheet_id,
            "google_sheet_tab": tab, "message": "เปลี่ยนฐานข้อมูลและรีเซ็ตข้อมูลแล้ว"}
...
def database_sync(request: Request) -> dict[str, Any]:
    _require(request)
    try:
        db.sync_selected_backend_now()
    except ValueError as exc:
        raise HTTPException(400, str(exc))
    return {"ok": True, "message": "ซิงก์ข้อมูลไปยังฐานข้อมูลที่เลือกแล้ว",
