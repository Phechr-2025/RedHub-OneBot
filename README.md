# Bio-shop Bot v36.0 — Telegram + Web Control

## Package inbound and auto-order v36.0

- หน้าเครือข่ายแสดงชื่อในกรอบเพียงจุดเดียว พร้อม ID เครือข่าย
- เครือข่ายไม่มี Inbound ID ในหน้าจัดการ; Inbound ID ย้ายไปอยู่ในแต่ละแพ็กเกจ
- ลำดับเครือข่ายและแพ็กเกจเริ่มที่ 1
- หลังเพิ่ม แก้ไข หรือลบ ระบบจัดลำดับใหม่เป็น 1..N อัตโนมัติ
- เมนู `/startadmin` แสดงจำนวนเครือข่ายและแพ็กเกจตามระบบใหม่
- Build marker: `v36.0-package-inbound-order`

## Network packages v35.0

- ค่าเริ่มต้นมีเฉพาะเครือข่าย AIS และ TRUE ไม่มีแพ็กเกจเริ่มต้น
- ต้องเพิ่มแพ็กเกจให้แต่ละเครือข่ายผ่านเว็บไซต์ก่อนผู้ใช้ซื้อได้
- ลำดับ `/addclient`: เครือข่าย → แพ็กเกจ → ชื่อไฟล์ → วัน → GB → DM
- หากเครือข่ายไม่มีแพ็กเกจ บอทจะแจ้ง `ยังไม่มีรายการแพ็กเกจ`
- แพ็กเกจตั้งชื่อ รายละเอียด ราคาต่อวัน สถานะ และลำดับได้
- Build marker: `v35.0-network-packages`

## Network flow v34.0

- ยกเลิกการเลือกแพ็กเกจก่อนซื้อ
- เครือข่ายเริ่มต้น AIS และ TRUE โดย Inbound ID/ลำดับเริ่มตั้งแต่ 1 ไม่ใช้ค่า 0
- เพิ่ม ลบ แก้ไข เปิด/ปิด และเรียงเครือข่ายได้จากเว็บไซต์
- ลำดับ `/addclient`: เลือกเครือข่าย → ตั้งชื่อไฟล์ → จำนวนวัน → GB → ส่ง DM
- ราคาคำนวณจาก `price_per_day × จำนวนวัน`
- Build marker: `v34.0-network-flow`

## Build fix v33.3

- เปลี่ยน Railway builder เป็น Dockerfile โดยตรง เพื่อตัดปัญหา Nixpacks ตรวจโปรเจกต์เป็น ASGI ผิดและลดเวลาวิเคราะห์ Build
- ย้าย health check เข้า `[deploy]` ตามรูปแบบ Railway
- เพิ่ม `Dockerfile`, `.dockerignore` และ `Procfile`; ลบ `railway.toml.txt` ที่ซ้ำซ้อน
- Docker cache dependencies แยกจาก source ทำให้ Build ครั้งถัดไปเร็วขึ้น
- Build marker ใหม่: `v33.3-buildfix`
